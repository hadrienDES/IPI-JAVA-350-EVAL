...
public class Employe {
    ...
    public Integer getNbRtt(LocalDate d){
        int jAnnee = d.isLeapYear() ? 365 : 366;
        int jAnneeNonTravaillé = 106;
        switch (LocalDate.of(d.getYear(),1,1).getDayOfWeek()){
            case THURSDAY: if(d.isLeapYear()) jAnneeNonTravaillé =  jAnneeNonTravaillé + 1; break;
            case FRIDAY: if(d.isLeapYear()) jAnneeNonTravaillé =  jAnneeNonTravaillé + 2; else jAnneeNonTravaillé =  jAnneeNonTravaillé + 1;
            case SATURDAY: jAnneeNonTravaillé = jAnneeNonTravaillé + 1; break;
        }
        int monInt = (int) Entreprise.joursFeries(d).stream().filter(localDate -> localDate.getDayOfWeek().getValue() <= DayOfWeek.FRIDAY.getValue()).count();
        return (int) Math.ceil((jAnnee - Entreprise.NB_JOURS_MAX_FORFAIT - jAnneeNonTravaillé - Entreprise.NB_CONGES_BASE - monInt) * tempsPartiel);
    }
    ...
    public void augmenterSalaire(Integer pourcentage) throws EmployeException{
        if (this.getSalaire() == 0) {
            throw new EmployeException("salaire égal à 0 erreur");
        }
        else if(pourcentage <= 0){
            throw new EmployeException("pourcentage inf à 0 erreur");
        }
        else {
